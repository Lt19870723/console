	...
	<table id="dataTable" class="fulltable" style="width: 100%;">
		<thead>
			<tr>
				...
				<th width="15%">地址</th>
				<th width="5%">状态</th>
				<th width="12%">最近推送时间</th>
				<th width="6%">推送人数</th>
				<th width="8%">推送成功人数</th>
				<th width="12%">操作</th>
			</tr>
		</thead>
	</table>
...
<script type="text/javascript">
	...
	*/
	function pageGo(pageNo) {
		...
		var urlPath = '${path}/maintain/xw/wxpushmore/list/' + pageNo + '.html';
	...
	*/
	...
	function add() {
		var _url = '${path}/maintain/xw/wxpushmore/add.html';
		$.layer({
			type : 2,
			title : '新增-多图文消息',
			area : [ '900px', '530px' ],
	...
	function detail(id) {
		var _url = '${path}/maintain/xw/wxpushmore/detail/' + id + '.html';
		$.layer({
			type : 2,
			title : '多图文详情',
			area : [ '900px', '530px' ],
	...
	function doDelete(id) {
		...
		$.ajax({
			url : '${path}/maintain/xw/wxpushmore/delete/' + id + '.html',
			type : 'post',
			dataType : 'json',
			success : function(result) {
				if (result.code == '0') {
	...
	function push(id) {
		...
		var _load = layer.load('处理中..');
		$.ajax({
			url : '${path}/maintain/xw/wxpushmore/push/' + id + '.html',
			type : 'post',
			dataType : 'json',
			success : function(result) {
				if (result.code == '0') {
	...
	function pushAgain(id) {
		...
		var _load = layer.load('处理中..');
		$.ajax({
			url : '${path}/maintain/xw/wxpushmore/pushagain/' + id + '.html',
			type : 'post',
			dataType : 'json',
			success : function(result) {
				if (result.code == '0') {
	...
	function toAddModify(id) {
		var _url = '${path}/maintain/xw/wxpushmore/modify/' + id + '.html';
		$.layer({
			type : 2,
			title : '修改-单图文消息',
			area : [ '900px', '530px' ],
