...
<body>
	<div style="margin-left: 20px; line-height: 50px;">
		菜单名称：<input type="text" id="name" name="name" class="input1" style="width: 180px;" />&nbsp;
		菜单层级：
		<select id="level" name="level" class="bigselect" size="1" style="width: 100px;">
...
function pageGo(pageNo) {
	...
	$.ajax({
		url : '${path}/system/menu/list/' + pageNo + '.html',
		data : {
			'name' : $.trim($('#name').val()),
			'level' : $('#level').val(),
			'status' : $('#status').val()
...
function edit(menuId) {
	var _url = '${path}/system/menu/edit.html';
	if (menuId != '') {
		_url += '?menuId=' + menuId;
	}
	$.layer({
...
function updateStatus(menuId) {
	layer.confirm("确认禁用?", function() {
		$.ajax({
			url : '${path}/system/menu/updateStatus/' + menuId + '.html',
			type : 'post',
			dataType : 'json',
			beforeSend:function(){
