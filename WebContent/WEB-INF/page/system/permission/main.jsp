...
<body>
	<div style="margin-left: 20px; line-height: 50px;">
		权限名称：<input type="text" id="name" name="name" class="input1" style="width: 180px;" />&nbsp;
		状态：
...
function pageGo(pageNo) {
	...
	$.ajax({
		url : '${path}/system/permission/list/' + pageNo + '.html',
		data : {
			'name' : $.trim($('#name').val()),
			'status' : $('#status').val()
		},
...
function edit(permissionId) {
	var _url = '${path}/system/permission/edit.html';
	if (permissionId != '') {
		_url += '?permissionId=' + permissionId;
	}
	$.layer({
		type : 2,
		title : '编辑',
...
function updateStatus(permissionId) {
	layer.confirm("确认禁用?", function() {
		$.ajax({
			url : '${path}/system/permission/updateStatus/' + permissionId + '.html',
			type : 'post',
			dataType : 'json',
			beforeSend:function(){
