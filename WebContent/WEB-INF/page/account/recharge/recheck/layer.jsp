	...
	<form id="approForm" action="" method="post">
		<table>
			...
			<tr>
				...
				<td colspan="3" width="350px"><textarea id="verifyRemark2" name="verifyRemark2"
						
						style="width:350px;height:100px;" >${rechargeRecordVo.verifyRemark2}</textarea></td>
			</tr>
			<tr>
				<td></td>
				<td colspan="3"> 
		...
		<input type="hidden" name="payment" type="hidden;" value="${rechargeRecordVo.payment }" />
...
function approPass(){
	...
	$("#approForm").ajaxSubmit ({
		url : '${path}/account/recheckaudit/pass.html',
		type : 'post',
		dataType : 'json',
...
function approReject(){
	var _load = parentLayer.load('处理中..');
	$("#approForm").ajaxSubmit ({
		url : '${path}/account/recheckaudit/reject.html',
		type : 'post',
		dataType : 'json',
		success : function(result) {
			if(result.code == '0'){
