		...
		<div id="loginLogCard" style="margin-left:20px;line-height:50px;">
			提现时间：
				<input
					...
					styleClass="Wdate" value="${cashRecordCnd.beginTime }">
				<f:convertDateTime pattern="yyyy-MM-dd"></f:convertDateTime>
				</input>
			至
				<input
					name="endTime" id="endTime" onclick="WdatePicker()"
					styleClass="Wdate" value="${cashRecordCnd.endTime }">
				<f:convertDateTime pattern="yyyy-MM-dd"></f:convertDateTime>
				</input>
			&nbsp;	
			用户名：<input id="username" name="username"
				value="${cashRecordCnd.username}" class="input1"/>
			&nbsp;	
			<br/>
			打款时间：<input
					name="beginTime2" id="payBeginTime" onclick="WdatePicker()"
					styleClass="Wdate" value="${cashRecordCnd.beginTime2 }">
				<f:convertDateTime pattern="yyyy-MM-dd"></f:convertDateTime>
				</input>
			至<input
					name="endTime2" id="payEndTime" onclick="WdatePicker()"
					styleClass="Wdate" value="${cashRecordCnd.endTime2 }">
				<f:convertDateTime pattern="yyyy-MM-dd"></f:convertDateTime>
				</input>
			&nbsp;	
			审核人：<input id="verifyName" name="verifyName"
				value="${cashRecordCnd.verifyName}" class="input1"/>
			&nbsp;	
			<br/>
			审核时间：<input
					name="verifyTimeBeginDate" id="verifyTimeBeginDate" onclick="WdatePicker()"
					styleClass="Wdate" value="${cashRecordCnd.verifyTimeBeginDate }">
				<f:convertDateTime pattern="yyyy-MM-dd"></f:convertDateTime>
				</input>
			至<input
					name="verifyTimeEndDate" id="verifyTimeEndDate" onclick="WdatePicker()"
					styleClass="Wdate" value="${cashRecordCnd.verifyTimeEndDate }">
				<f:convertDateTime pattern="yyyy-MM-dd"></f:convertDateTime>
				</input>
			&nbsp;	
			状态 ：
