...
<form id="insertform">
	<div id="tab1" style="height:300px;">
		...
		<div class="left1_input_ts" style="float: left;width:600px;">
			...
			<div style="margin-top:10px;font-size:14px;">
				<input type="text" class="input1" id="branchName" name="branchName" value="${BankVo.branchName }" maxlength="50" style="width:320px;"/>
				<input type="hidden" id="bankVoId" value="${BankVo.id }"/>
			</div>
		</div>
		<div class="left1_input_ts" style="float: left;width:600px;">
			<span class="input_span">联行号：</span>
			<div style="margin-top:10px;font-size:14px;">
				<input type="text" class="input1" id="cnapsCode" name="cnapsCode" value="${BankVo.cnapsCode }" maxlength="50" style="width:320px;"/>
			</div>
		</div>
		<div class="left1_input_ts" style="float: left;width:600px;">
			<span class="input_span">状态：</span>
			<div style="margin-top:10px;font-size:14px;">
				<select id="status" class="bigselect" style="width:320px;">
					<option value="0" <c:if test="${BankVo.status==0}">selected="selected"</c:if>>正常</option>
					<option value="-1" <c:if test="${BankVo.status==-1}">selected="selected"</c:if>>失效</option>
				 </select>
			</div>
		</div>
	...
	$.ajax({
		...
		success : function(result) {
			...
			for (var i = 0; i < data1.length-1; i++) {
				...
				if("${BankVo.province}"==data2[0]){
					$("#province").append("<option selected='selected' value=\"" + data2[0] + "\">" + data2[1] + "</option>");
				}else{
...
function initBankList(){
	$.ajax({
		...
		success : function(result) {
			...
			for (var i = 0; i < data1.length-1; i++) {
				var data2 = data1[i].split(',');
				if("${BankVo.bankName}"==data2[0]){
					$("#bankName").append("<option selected='selected' value=\"" + data2[0] + "\">" + data2[1] + "</option>");
				}else{
