...
<div style="margin-left:10px;">
 待还总额：<span style="color:red;"><fmt:formatNumber type="currency" value="${sumRepayAccount}" currencySymbol="¥" maxFractionDigits="2"/>  </span> 
</div>
	...
	<table id="dataTable" class="fulltable" style="width: 100%;">
		...
		<c:forEach items="${page.result }" var="vo" varStatus="sta">
			...
			<td>
				...
				<c:if test="${vo.status == 0 and vo.webstatus == 5}">
					银行垫付中
				</c:if>
			</td>
			<td>
			...
			<td>
			    <c:if test="${vo.webstatus==0}">
			           <a href="javascript:;" onclick="webpay(${vo.id});">垫付</a>
			    </c:if>
				<c:if test="${vo.webstatus==5}">
					<a href="javascript:;" onclick="exportToExcel(${vo.id});">导出垫付中</a>
				</c:if>
			</td>
			<td>
				<c:if test="${vo.webstatus==5}">
					<a href="javascript:;" onclick="webpayConfirm(${vo.id});">银行垫付确认</a>
				</c:if>
			</td>
			</tr>
		</c:forEach>
