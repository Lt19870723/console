...
function pageGo(pageNo) {
	...
	$("#queryForm").ajaxSubmit ({
		url : '${path}/manage/forRepaymentFailBorrow/list/' + pageNo + '.html',
		type : 'post',
		dataType : 'html',
...
function validateExportDataCount(){
	$("#queryForm").ajaxSubmit ({
		url : '${path}/manage/forRepaymentFailBorrow/count.html',
		type : 'post' ,
		dataType : 'json',
		success : function(result) {
			...
			if(result.code == '0'){
				//数据导出
				$("#queryForm").attr("action","${path}/manage/forRepaymentFailBorrow/export.html");
		        $("#queryForm").submit();
			}else{
				parentLayer.close(_load);
