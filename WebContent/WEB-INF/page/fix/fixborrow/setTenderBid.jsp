			...
			<tr>
				...
				<td>
					...
					<input type="radio" value="0" name="tenderBidFlag" onclick="$('#tenderBidDate').attr('disabled','disabled');"
						<c:if test="${fixBorrowVo.tenderBidFlag!=1 }">checked="checked"</c:if>>不限制
				</td>
			</tr>
			<tr>
				<td align="right">限制自动投标截止日期：</td>
				<td>
					<input id="tenderBidDate" name="tenderBidDate" onclick="WdatePicker({minDate:'%y-%M-{%d+1}'})" styleClass="Wdate" value="<fmt:formatDate value='${fixBorrowVo.tenderBidDate }' pattern='yyyy-MM-dd'/>" 
						<c:if test="${fixBorrowVo.tenderBidFlag!=1 }">disabled="disabled"</c:if>>
