...
<body>
	...
	<table style="margin-left: 10px;margin-right: 10px; margin-top: 5px;">
		<tr>
	       <td align="right" width="25%">第三方平台：</td>
	       <td>
	       		<c:if test="${journal.thirdPlatform == 1}">网银在线</c:if>
				<c:if test="${journal.thirdPlatform == 5}">连连支付</c:if>
				<c:if test="${journal.thirdPlatform == 6}">富友支付</c:if>
				<c:if test="${journal.thirdPlatform == 4}">新浪支付</c:if>
				<c:if test="${journal.thirdPlatform == 2}">国付宝</c:if>
				<c:if test="${journal.thirdPlatform == 3}">盛付通</c:if>
	       </td>
	    </tr>
		<tr>
