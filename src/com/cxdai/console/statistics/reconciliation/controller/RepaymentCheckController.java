 ...
 */
@Controller
@RequestMapping(value="/statistics/repaymentcheckinfo")
public class RepaymentCheckController extends BaseController {
	...
	@RequestMapping("/main")
	public ModelAndView forRepaymentCheckMain(){
		return new ModelAndView("/statistics/reconciliation/repaymentcheck/main");
	}
	 ...
	 * @return
	 */
	@RequestMapping("/repaymentcheckcount")
	public ModelAndView queryRepaymentCheckMap(@ModelAttribute AccountCheckCnd accountCheckCnd,HttpServletRequest request){
		...
		try {
			...
			checkMap = accountCheckService.queryRepaymentCheckByCnd(accountCheckCnd);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("根据条件查询资金对账数据(还款对账)出错", e);
		}
		return new ModelAndView("/statistics/reconciliation/repaymentcheck/list").addObject("checkMap", checkMap);
	}
