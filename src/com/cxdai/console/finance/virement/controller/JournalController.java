...
public class JournalController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping("/journalMain")
	public ModelAndView journalMain() {
		return new ModelAndView("/finance/journal/journalMain");
	}

	@RequestMapping("/journalList/{pageNo}")
	public ModelAndView searchPageJournalList(@ModelAttribute JournalCnd journalCnd, @PathVariable Integer pageNo) {
		...
		try {
			...
			journalCnd.setType(Journal.TYPE_JOURNAL);
			page = journalService.queryListForPage(journalCnd, pageNo, Constants.CONSOLE_PAGE_SIZE);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error(e);
		}
		return new ModelAndView("/finance/journal/journalList").addObject("page", page);
	}

	@RequestMapping("/addJournal")
	public ModelAndView addJournal(Integer id) {
		ModelAndView mv = new ModelAndView("/finance/journal/addJournal");
		if (id != null) {
	...
	public MessageBox saveOrUpdateJournal(@ModelAttribute JournalVo journalVo) {
		try {
			journalVo.setType(Journal.TYPE_JOURNAL);
	...
	public ModelAndView detail(@PathVariable Integer id) {
		ModelAndView mv = new ModelAndView("/finance/journal/detailJournal");
		if (id != null) {
			Journal journal = journalService.selectByPrimaryKey(id);
			mv.addObject("journal", journal);
		}
	 ...
	 * @throws IOException
	 */
	@RequestMapping(value = "downloadJournalExcel")
	public void downloadJournalExcel(@ModelAttribute JournalCnd journalCnd, HttpServletRequest req,
		...
		try {
			...
			file = WordAndExcelGenerator.createDocOrExcel("现金日记账", map, "journal");
			...
			String fileName = new String("现金日记账.xls".getBytes("GBK"), "iso8859-1");
	...
	public ModelAndView searchJournalMain() {
		return new ModelAndView("/finance/journal/searchJournalMain");
	}
	...
	public ModelAndView searchJournalList(@ModelAttribute JournalCnd journalCnd, @PathVariable Integer pageNo) {
		...
		return new ModelAndView("/finance/journal/searchJournalList").addObject("page", page);
	}
}
